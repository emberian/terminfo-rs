...
pub static boolfnames: &'static[&'static str] = &'static["auto_left_margin", "auto_right_margin",
    ...
    "cpi_changes_res", "lpi_changes_res" ];
...
pub fn parse(file: @Reader, longnames: bool) -> Result<~TermInfo, ~str> {
    ...
    let names_str = str::from_bytes(file.read_bytes(names_bytes as uint));
    ...
    for string_offsets.eachi |i, v| {
        ...
        match nulpos {
            Some(x) => {
                ...
                string_map.insert(name.to_owned(), str::from_bytes(string_table.slice(offset as uint, x)))
            },
            None => {
